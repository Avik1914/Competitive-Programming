...
long hashMap(String[] queryType, int[][] query) {
    long globalKeyAdd=0l;
    long globalValueAdd=0l;
    Map<Integer,Integer> map=new HashMap();
    int len=queryType.length;
    long sum=0;
    for(int i=0;i<len;i++){
        if(queryType[i].equals("insert"))
            map.put(query[i][0]-(int)globalKeyAdd,query[i][1]-(int)globalValueAdd);
        else if(queryType[i].equals("get"))
            sum+=map.getOrDefault(query[i][0]-(int)globalKeyAdd,0)+globalValueAdd;
        else if(queryType[i].equals("addToValue"))
            globalValueAdd+=query[i][0];
        else
            globalKeyAdd+=query[i][0];
    }
    return sum;
}
